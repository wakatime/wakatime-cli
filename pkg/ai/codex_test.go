...
func TestCodexParse(t *testing.T) {
	...
	assert.Equal(
		t,
		float64(time.Date(2026, 3, 28, 11, 33, 14, int(289*time.Millisecond), time.UTC).UnixMilli())/1000,
		got[0].Time,
	)
	...
	assert.Equal(
		t,
		float64(time.Date(2026, 3, 28, 11, 33, 18, int(535*time.Millisecond), time.UTC).UnixMilli())/1000,
		got[1].Time,
	)
