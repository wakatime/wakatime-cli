...
func Execute() {
	...
	flags.Int32Var(&args.Editor.Lineo, "lineo", math.MinInt32, "Optional line number. This is the current line being edited.")
	flags.Int32Var(&args.Editor.CursorPos, "curorpos", math.MinInt32, "Optional cursor position in the current file.")
	...
	flags.StringVar(&args.Proxy.SslCertsFile, "ssl-certs-file", "", "Override the bundled Python Requests CA certs file. By default, uses certifi for ca certs.") //Rename python in the string as soon as it's implemented
	...
	flags.BoolVar(&args.Obfuscate.HideProjectNames, "hide-project-names", false, "Obfuscate project names. When a project older is detected instead of using the folder name as the project, a .wakatime-project file is created with a random project name.")
...
func runLegacy(args legacy.Arguments, obsoleteArgs legacy.ObsoleteArguments, cmd *cobra.Command) {
	...
	if !flags.Changed("include-only-with-project-file") {
		...
		if err == nil {
			...
			args.Include.IncludeOnlyWithProjectFile = b == true
		}
	}
